...
export class WhatsappWebhookService {
  ...
  async handleIncomingMessage(payload: any): Promise<void> {
    ...
    if (message === 'PING') {
      await this.extNotificationService.sendWhatsApp(
        sender,
        'PONG! Sistem APSS Pesantren terhubung. ✅',
      );
      return;
    }
    ...
    if (!wali) {
      ...
      if (message.startsWith('INFO') || message.startsWith('SALDO')) {
        await this.extNotificationService.sendWhatsApp(
          sender,
          'Maaf, nomor Anda belum terdaftar di sistem Pesantren kami.',
        );
      }
      return;
    }
    ...
    if (message === 'INFO') {
      ...
      await this.extNotificationService.sendWhatsApp(
        sender,
        `Halo Bpk/Ibu ${wali.name}, Anda terdaftar sebagai Wali dari: ${santriNames}. Ketik SALDO untuk info uang saku.`,
      );
      return;
    }
    ...
    await this.extNotificationService.sendWhatsApp(
      sender,
      'Perintah tidak dikenali. Ketik INFO untuk bantuan.',
    );
  }
}
